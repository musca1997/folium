...
export function SummaryToggle({ summary, summaryZh, fallback, multilingualEnabled = false, preferredLanguage = "en" }: SummaryToggleProps) {
  const hasZh = multilingualEnabled && Boolean(summaryZh?.trim());
  const [language, setLanguage] = useState<"en" | "zh">(hasZh && preferredLanguage === "zh" ? "zh" : "en");
  const text = useMemo(() => {
